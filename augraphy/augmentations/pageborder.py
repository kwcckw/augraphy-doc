...
class PageBorder(Augmentation):
    ...
    def create_page_borders(
        ...
        if self.same_page_border:
            ...
            border_image_merged = np.full_like(image_rotated, fill_value=255,dtype="uint8")
        else:
            border_image_merged = np.full((image_rotated.shape[0]+border_height, image_rotated.shape[1]+border_width, 3), fill_value=255,dtype="uint8")
        ...
        for i in reversed(range(total_shifts)):
            ...
            # for condition where border width is 0
                ...
                border_image_merged[shifted_value_y:-border_height+shifted_value_y, :][indices] = border_image_single[indices]
            # for condition where border height is 0
            elif shifted_value_y-border_height == 0:
                border_image_merged[:, shifted_value_x:-border_width + shifted_value_x][indices] = border_image_single[indices]
            # for condition where border width and height are not 0
            else:        
                border_image_merged[shifted_value_y:-border_height+shifted_value_y, shifted_value_x:-border_width + shifted_value_x][indices] = border_image_single[indices]
    ...
    def __call__(self, image, layer=None, force=False):
        if force or self.should_run():
            ...
            if self.page_border_width_height == "random":
                ...
                if self.page_border_width_height[0] <= 1 and self.page_border_width_height[0] >= -1 and isinstance(self.page_border_width_height[0], float):
                    border_width = int(self.page_border_width_height[0]* width)
                else:
                    border_width = self.page_border_width_height[0]
                    
                if self.page_border_width_height[1] <= 1 and self.page_border_width_height[1] >= -1 and isinstance(self.page_border_width_height[1], float):
